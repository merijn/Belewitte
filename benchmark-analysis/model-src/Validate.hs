...
validateModel :: Key GPU -> Model -> TrainingConfig -> SqlM ()
validateModel gpuId model config = do
    validation <- getValidationQuery gpuId config
  ...
  where
    ...
    total = getTotalQuery gpuId config
